      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="relative group">
            <textarea className="w-full bg-slate-50 border-2 border-transparent focus:border-emerald-500 rounded-3xl p-6 h-40 outline-none text-sm font-bold text-slate-800 placeholder:text-slate-300 resize-none shadow-inner" placeholder="যেমন: x + 1 = 5 হলে x কত?" value={problem} onChange={(e) => { setProblem(e.target.value); setCapturedImage(null); }} />
            <button onClick={startListening} className={`absolute bottom-4 right-4 w-10 h-10 rounded-xl flex items-center justify-center shadow-lg transition-all ${isRecording ? 'bg-rose-500 text-white animate-pulse' : 'bg-white text-emerald-600 border border-emerald-50'}`}>🎤</button>
          </div>
          <div className={`h-40 rounded-3xl border-2 border-dashed flex flex-col items-center justify-center gap-2 cursor-pointer transition-all overflow-hidden relative ${capturedImage ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`} onClick={() => fileInputRef.current?.click()}>
            {capturedImage ? <img src={capturedImage} className="w-full h-full object-contain p-2" alt="Problem" /> : <><div className="text-2xl">📷</div><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">ছবি আপলোড</span></>}
            <input type="file" accept="image/*" capture="environment" className="hidden" ref={fileInputRef} onChange={handleFileSelect} />
          </div>
        </div>

        <button onClick={handleSolve} disabled={loading || (!problem.trim() && !capturedImage)} className="w-full bg-emerald-600 text-white h-16 rounded-3xl font-black text-lg hover:bg-emerald-700 shadow-xl transition-all disabled:opacity-50">
