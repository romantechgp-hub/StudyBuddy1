    ...
    <div className="bg-white rounded-[2.5rem] p-6 sm:p-12 shadow-xl border border-slate-50 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl shadow-sm">
          <button onClick={() => setMode('brief')} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'brief' ? 'bg-teal-600 text-white' : 'text-slate-400'}`}>সংক্ষিপ্ত</button>
          <button onClick={() => setMode('detailed')} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'detailed' ? 'bg-teal-600 text-white' : 'text-slate-400'}`}>বিস্তারিত</button>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {script && (
          <div className="mt-8 p-6 bg-teal-50 rounded-[2.5rem] border border-teal-100 animate-in zoom-in duration-500 shadow-sm">
            <h4 className="text-teal-800 font-black mb-6 flex items-center gap-2"><span>📝</span> আপনার স্ক্রিপ্ট ({mode === 'brief' ? 'সংক্ষিপ্ত' : 'বিস্তারিত'}):</h4>
