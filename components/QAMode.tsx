    ...
    <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-xl border border-slate-100 max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl shadow-sm">
          <button onClick={() => setMode('brief')} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'brief' ? 'bg-rose-600 text-white' : 'text-slate-400'}`}>সংক্ষিপ্ত</button>
          <button onClick={() => setMode('detailed')} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'detailed' ? 'bg-rose-600 text-white' : 'text-slate-400'}`}>বিস্তারিত</button>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {result && (
          <div className="mt-8 p-6 bg-rose-50/50 rounded-[2.5rem] border border-rose-100 shadow-sm animate-in zoom-in duration-500">
            <h4 className="font-black text-rose-800 flex items-center gap-2 mb-4"><span>✍️</span> উত্তর ({mode === 'brief' ? 'সংক্ষিপ্ত' : 'বিস্তারিত'}):</h4>
